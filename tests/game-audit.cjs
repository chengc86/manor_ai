...
for(const n of ['builds','equipment','heroes','chapters','battle','clothing','questions','year6-expansion','mixed-questions','progression-questions','reading-expansion','question-rewards','year2-questions','fresh-questions','textbook-questions','quest-expansion'])fs.writeFileSync(`work/${n}.cjs`,ts.transpileModule(fs.readFileSync(`lib/${n}.ts`,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText.replace(/require\("\.\/(.*?)"\)/g,'require("./$1.cjs")'));
