...
for(const n of ['builds','equipment','heroes','chapters','battle','clothing','questions','year6-expansion','mixed-questions','progression-questions','reading-expansion','question-rewards','year2-questions','fresh-questions','textbook-questions','quest-expansion','more-questions'])fs.writeFileSync(`work/${n}.cjs`,ts.transpileModule(fs.readFileSync(`lib/${n}.ts`,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText.replace(/require\("\.\/(.*?)"\)/g,'require("./$1.cjs")'));
...
const expansion=questions.filter(q=>q.id.startsWith('quest-400-'));assert.equal(expansion.length,400);for(const subject of ['Maths','English','Verbal reasoning','Non-verbal reasoning'])assert.equal(expansion.filter(q=>q.subject===subject).length,100);for(const q of expansion){assert.equal(q.difficulty,'Year 6');if(q.diagram)assert(fs.existsSync(`public/question-diagrams/photo-${q.diagram.items[0]}.svg`));if(q.options)assert.equal(new Set(q.options).size,q.options.length);}console.log('PASS: 400 new questions, 100 per subject, valid choices and all diagrams present.');
