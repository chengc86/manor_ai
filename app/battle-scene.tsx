...
import {simulate,monsterHealth,percent,pathPosition,COLS,ROWS,PATH,type Battle} from '@/lib/battle';
...
export default function BattleScene({battle,now}:{battle:Battle;now:number}){
 ...
 const monsters=Array.from({length:sim.count},(_,i)=>{const progress=(elapsed-i*sim.rule.spawn)/sim.rule.travel;const hp=Math.max(0,monsterHealth(battle.wave,i)-sim.events.filter(e=>e.monster===i&&e.at+.25<=elapsed).reduce((s,e)=>s+e.damage,0));const d=sim.deathAt[i];return{id:i,hp,maxHp:monsterHealth(battle.wave,i),boss:sim.rule.boss&&i===sim.count-1,progress,pos:visualPosition(((d===null?elapsed:Math.min(elapsed,d+.25))-i*sim.rule.spawn)/sim.rule.travel),dead:hp===0,deathAt:d===null?null:d+.25}});
