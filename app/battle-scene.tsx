...
export default function BattleScene({battle,now}:{battle:Battle;now:number}){
 ...
 {battle.fighters.map((f,i)=>{const pos=percent(f.cell),attack=active.find(e=>e.fighter===i);return <div className={`battle-fighter type-${f.type} tier-${f.level} ${attack?'attacking':''}`} key={f.id} style={{left:`${pos.x}%`,top:`${pos.y}%`,'--hero-colour':colours[f.type]} as CSSProperties}><span className="hero-pedestal"/><span className={`sprite sprite-${f.type}`}/><small className="battle-owner">{f.name}</small><span className="fighter-level">{f.level}</span>{attack&&<span className="attack-name">{HEROES[f.type].attack}</span>}</div>})}
