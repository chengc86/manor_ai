...
export default function BattleScene({battle,now}:{battle:Battle;now:number}){
 ...
 {monsters.map(m=>{if(m.progress<0||m.progress>=1||m.deathAt!==null&&elapsed>m.deathAt+.75)return null;const hit=active.find(e=>e.monster===m.id&&elapsed-e.at>=.25);const before=visualPosition(m.progress-.003),after=visualPosition(m.progress+.003),facing=after.x<before.x?-1:1;return <div key={m.id} className={`battle-monster ${m.dead?'vanquished':''} ${hit?'hit':''}`} style={{left:`${m.pos.x/COLS*100}%`,top:`${m.pos.y/ROWS*100}%`}}><span className="monster-shadow"/><span className="monster-facing" style={{transform:`scaleX(${facing})`}}><span className={`sprite sprite-enemy monster-variant-${m.id%3}`} style={{animationDelay:`-${m.id*.137}s`}}/></span><span className="monster-hp-track"><span style={{width:`${m.hp/sim.rule.hp*100}%`}}/></span>{hit&&!m.dead&&<b className="damage-number">−{hit.damage}</b>}{m.dead&&<b className="poof">✦</b>}</div>})}
