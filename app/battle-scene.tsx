...
export default function BattleScene({battle,now}:{battle:Battle;now:number}){
 ...
 {battle.fighters.map((f,i)=>{const pos=percent(f.cell),attack=active.find(e=>e.fighter===i);return <div className={`battle-fighter type-${f.type} tier-${f.level} ${attack?'attacking':''}`} key={f.id} style={{left:`${pos.x}%`,top:`${pos.y}%`,'--hero-colour':f.colour??colours[f.type]} as CSSProperties}><span className="hero-pedestal"/><Sprite type={f.type} gender={f.gender} uniform={f.uniform} clothing={f.clothing} outfit={f.outfit} weapon={f.weapon??(f.level?'standard':'none')}/><small className="battle-owner" style={{border:`2px solid ${f.colour??colours[f.type]}`}}>{f.name}</small><span className="fighter-level">{f.level}</span>{attack&&<span className="attack-name">{equipment(f.weapon).effect.toUpperCase()}</span>}</div>})}
