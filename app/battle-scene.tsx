...
export default function BattleScene({battle,now}:{battle:Battle;now:number}){
 ...
 {battle.fighters.map((f,i)=>{const pos=percent(f.cell),attack=active.find(e=>e.fighter===i);return <div className={`battle-fighter type-${f.type} tier-${f.level} ${attack?'attacking':''}`} key={f.id} style={{left:`${pos.x}%`,top:`${pos.y}%`,'--hero-colour':f.colour??colours[f.type]} as CSSProperties}><span className="hero-pedestal"/><Sprite type={f.type} outfit={f.outfit} weapon={f.weapon??(f.level?'standard':'none')}/><small className="battle-owner" style={{border:`2px solid ${f.colour??colours[f.type]}`}}>{f.name}</small><span className="fighter-level">{f.level}</span>{attack&&<span className="attack-name">{f.weapon==='frost'?'FREEZE!':f.weapon==='blast'?'BURST!':HEROES[f.type].attack}</span>}</div>})}
 {monsters.map(m=>{if(m.progress<0||m.progress>=1||m.deathAt!==null&&elapsed>m.deathAt+.75)return null;const hit=active.find(e=>e.monster===m.id&&elapsed-e.at>=.25);const frozen=monsterHits[m.id].some(e=>e.freeze&&elapsed>=e.at&&elapsed<e.at+e.freeze);const before=visualPosition(m.progress-.003),after=visualPosition(m.progress+.003),facing=after.x<before.x?-1:1;return <div key={m.id} className={`battle-monster ${frozen?'frozen':''} ${m.boss?'boss-monster':''} ${m.dead?'vanquished':''} ${hit?'hit':''}`} style={{left:`${m.pos.x/COLS*100}%`,top:`${m.pos.y/ROWS*100}%`}}><span className="monster-shadow"/>{m.boss&&<span className="boss-label">BOSS</span>}<span className="monster-facing" style={{transform:`scaleX(${facing})`}}><span className={`sprite sprite-enemy monster-variant-${m.id%3}`} style={{animationDelay:`-${m.id*.137}s`}}/></span><span className="monster-hp-track"><span style={{width:`${m.hp/m.maxHp*100}%`}}/></span>{hit&&!m.dead&&<b className="damage-number">−{hit.damage}</b>}{m.dead&&<b className="poof">✦</b>}</div>})}
